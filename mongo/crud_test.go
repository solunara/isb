...
func TestMongo(t *testing.T) {
	...
	// 连接mongodb
	opts := options.Client().ApplyURI("mongodb://root:example@192.168.110.249:27017").SetMonitor(monitor)
	client, err := mongo.Connect(ctx, opts)
	assert.NoError(t, err)

	mdb := client.Database("vbook")
	...
	fmt.Printf("id %v", res.InsertedID)
	...
	art = Article{}
	err = col.FindOne(ctx, bson.M{"id": 123}).Decode(&art)
	...
	sets := bson.D{{Key: "$set", Value: bson.D{{Key: "title", Value: "新的标题"}}}}
